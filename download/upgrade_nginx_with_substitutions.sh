...
function upgrade_nginx()
{
	...
	./configure --user=www --group=www --prefix=/usr/local/nginx --with-http_stub_status_module --with-http_ssl_module --with-http_gzip_static_module --with-ipv6 --with-pcre --with-http_sub_module --with-http_v2_module --add-module=../substitutions4nginx-read-only
	make
	...
	if [ "$add_substitution" != 'n' ]; then
		...
		if grep -q "^[ \t]*subs_filter" "nginx.conf"; then
 		...
 		wget "http://oa.shibang.cn/forbidden-word/output.php?type=nginx" -O /usr/local/nginx/conf/subs_filter.conf
 		if [ $? -eq 0 ]; then
